...
class EmbryoSupervisedDataset(Dataset):
    ...
    def _create_repeated_indices(self):
        ...
        if len(class_counts) < 2:
            return list(self.data_df.index) # Nema imbalance-a ili samo jedna klasa
            
        # Određivanje većinske i manjinske klase
        minority_class = class_counts.index[-1]
        
        # Liste za prikupljanje ponovljenih indeksa
        final_indices = []
        ...
        for idx in self.data_df.index:
            ...
            if class_label == minority_class:
                # Ponavljaj manjinsku klasu N puta
                final_indices.extend([idx] * self.minority_repeat_factor)
            else:
                # Većinsku klasu ponavljaj jednom
                final_indices.append(idx)
                
        return final_indices

    def __len__(self):
        # 💡 KLJUČNA IZMENA 2: Dataset sada ima dužinu efektivne liste indeksa
        return len(self.effective_indices)

    def __getitem__(self, idx):
        # 💡 KLJUČNA IZMENA 3: Učitavanje originalnog indeksa iz liste
        original_idx = self.effective_indices[idx]
        
        row = self.data_df.iloc[original_idx] 
        # ... (Ostatak __getitem__ metode je isti) ...
